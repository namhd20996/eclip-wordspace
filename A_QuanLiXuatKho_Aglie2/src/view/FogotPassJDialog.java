...
//		lbl_Exit.addMouseListener(ctl);
	...
	public void verifyTT() {
		...
		if (UserDao.getInstance().checkEmail(userM) != null) {
			// Lấy dãy số xác thực
			...
			userM.setThoiGianHieuLucMaXacThuc(thoiGianHieuLucMaXacThuc);
			userM.setTrangThaiXacThuc(trangThaiXacThuc);
			if (UserDao.getInstance().updateInformation(userM) > 0) {
	...
	public String getNoiDung(UserModel kh) {
		String link = "http://localhost:8080/BoostrapLearning/khach-hang?action=xac-thuc&maKhachHang=" + kh.getUser()
